...
void BuildCategoryNav(map<int,string> catemap){
    ...
    for(it_type iterator = catemap.begin(); iterator != catemap.end(); iterator++) {
        ...
        string url, name;
        crow::json::dump_internal(category_id[id]["url"], url);
        crow::json::dump_internal(category_id[id]["name"], name);
        if(iterator->first < 0){
...
void BuildCategoryIndex(){
    RestClient::response r = RestClient::get(DATABASE_URL"category/_design/category/_view/order");
    if(r.code == 200){
...
crow::json::wvalue render_topic_list_string(string list){
    ...
    for(int i=0; i < json["rows"].size(); i++)
    {
        ...
        string category;
        dump_internal(wv["category"], category);
        wv["categoryname"] = crow::json::dump(category_id[category]["name"]);
        wv["categoryurl"] = crow::json::dump(category_id[category]["url"]);
...
crow::json::wvalue render_topic_json(crow::json::rvalue &json){
    ...
    topic["categoryname"] = crow::json::dump(category_id[category]["name"]);
    topic["categoryurl"] = crow::json::dump(category_id[category]["url"]);
    return topic;
}
