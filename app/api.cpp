...
string user_info(string uid){
    RestClient::response r = RestClient::get(DATABASE_URL"user/" + uid, false);
    if(r.code == 200){
        return r.body;
    }else{
        return "error";
...
string get_notifications(string uid){
    RestClient::response r = RestClient::get(DATABASE_URL"notification/" + uid, false);
    if(r.code == 200){
        return r.body;
    }else{
        return "error";
...
int get_notification_count(string uid){
    RestClient::response r = RestClient::get(DATABASE_URL"notification/_design/get/_view/count?key=\"" + uid + "\"", false);
    if(r.code == 200){
        if(r.body.find("rows\":[\r\n\r\n]") == string::npos){
...
string user_info_by_name(string uname){
    RestClient::response r = RestClient::get(DATABASE_URL"user/_design/user/_view/username?key=\"" + uname + "\"", false);
    if(r.code == 200){
        if(r.body.find("rows\":[\r\n\r\n]") == string::npos){
            return r.body;
        }else{
...
string get_topic(string ID){
    RestClient::response r = RestClient::get(DATABASE_URL"topics/" + ID, false);
    if(r.code == 200){
        return r.body;
    }else{
        return "error";
...
string get_new_topic(int page,int category){
    ...
    RestClient::response r = RestClient::get(DATABASE_URL"topics/_design/get/_view/" + c + "?descending=true&limit=" + to_string(TOPIC_PER_PAGE) + "&skip=" + to_string(skip), false);
    if(r.code == 200){
        if(r.body.find("rows\":[\r\n\r\n]") == string::npos){
            return r.body;
        }else{
